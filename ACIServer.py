    ...
    async def connection_handler(self, websocket, path):
        ...
        while True:
            ...
            if cmd["cmd"] == "set_value":
                ...
                response = json.dumps({"cmd": "set_value", "msg": str(cmd["db_key"]) + "[" + str(cmd["key"]) + "] = " + str(newValue)})
                await websocket.send(response)

            if cmd["cmd"] == "get_index":
                response = json.dumps({"cmd": "get_index", "msg": self.dbs[cmd["db_key"]].data[cmd["key"]].get_index(cmd["index"], websocket.user), "key":cmd["key"], "db_key":cmd["db_key"]})
                await websocket.send(response)

            if cmd["cmd"] == "set_index":
                response = json.dumps({"cmd": "set_index", "msg": self.dbs[cmd["db_key"]].data[cmd["key"]].set_index(cmd["index"], cmd["value"], websocket.user), "key":cmd["key"], "db_key":cmd["db_key"]})
                await websocket.send(response)

            if cmd["cmd"] == "append_list":
                response = json.dumps({"cmd": "append_list", "msg": self.dbs[cmd["db_key"]].data[cmd["key"]].append_index(cmd["value"], websocket.user), "key":cmd["key"], "db_key":cmd["db_key"]})
                await websocket.send(response)

            if cmd["cmd"] == "get_list_length":
                response = json.dumps({"cmd": "get_list_length", "msg": self.dbs[cmd["db_key"]].data[cmd["key"]].get_len(websocket.user), "key":cmd["key"], "db_key":cmd["db_key"]})
                await websocket.send(response)

            if cmd["cmd"] == "get_recent":
                print("Client is requesting recent index")
                response = json.dumps({"cmd": "get_recent", "msg": self.dbs[cmd["db_key"]].data[cmd["key"]].get_recent(cmd["num"], websocket.user), "key":cmd["key"], "db_key":cmd["db_key"]})
                await websocket.send(response)
                print("sent")
